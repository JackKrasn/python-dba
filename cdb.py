...
if args.host != 'localhost':
    ...
    client.run('. /u/dba/venv/python_dba/bin/activate; python /tmp/'
             ...
             + ' -o ' + args.oradata + ' -n ' + args.nls_characterset + cdb + archive_mode)
    client.close()
 else:
    ...
    db = db.LocalDb(oracle_version)
    db.create_from_tpl(args.sid, oradata, nls=args.nls_characterset, archive_mode=args.archive_mode, cdb=args.cdb)
    # if args.type == 'cft':
    #     for sql in db.get_sql('create_tablespaces.sql'):
    #         db.cur().ddl_execute(sql)